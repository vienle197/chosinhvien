...
use common\models\LoginFormCustomer as LoginForm;
...
class LoginController extends ServiceController
{
    ...
    public function actionLogin()
    {
        ...
        if ($model->login()) {
            return $this->response(true,"Đăng nhập thành công!");
        } else {
            return $this->response(false,"Tài khoản hoặc mật khẩu không chính xác!",$model->errors);
        }
    }
...
//        }
        ...
        if ($user = $model->signup()) {
            if (Yii::$app->getUser()->login($user)) {
                return $this->response(true, 'Đăng ký thành công!');
            }
        }
        return $this->response(false, 'Có sự cố sảy ra!');
    }
